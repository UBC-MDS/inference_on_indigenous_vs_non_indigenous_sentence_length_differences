---
title: "Statistical siginificance in the aggregate sentence lengths of indigenous vs non-indigenuous racial groups"
...
counts <- read_csv('../results/EDA/group_counts.csv')
```
...
knitr::include_graphics('../results/EDA/eda-densities.png')
```

Looking at the boxplot, we noted the large overlap in the confidence intervals between the two groups, which at a glance indicated that a statistically significant differene was unlikely. 
...
knitr::include_graphics('../results/EDA/eda-box_plots.png')
```
...
The difference in the sample test medians was ###DIFF_IN_MEANS, which corresponded to a p-value of  ###P_VALUE. Since the test statistic falls in the left tail in the distribution plot
 and the two-tailed $p$-value = 0.0328<0.05, we can conclude that given the
 significant level of $\alpha = 0.05$, there is statistically significant
 evidence that we can reject $H_0$ such that there is
