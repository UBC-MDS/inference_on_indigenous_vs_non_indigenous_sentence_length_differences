...
All other columns were dropped. While there were mo missing values, some sentences are of indeterminate length, and could not be used in the analysis. We also dropped all the rows with indeterminate sentences. At the end of data processing, the count of observations in either group was `r counts$Count[2]` and `r format(counts$Count[1])`. This provided a large enough sample size.
...
Looking at the box plots, we noted the large overlap in the confidence intervals between the two groups, which at a glance indicated that a statistically significant difference was unlikely.
...
We had a large enough sample size to apply a permutation test as the total sample size was `r format(counts$Count[1] + counts$Count[2])`. We carried out the permutation test with 5,000 under the null hypothesis, computing and comparing the median for each of the two groups.
...
```{r Null Distribution 3, fig.width=10, fig.height=3, fig.cap = "Figure 3. The distribution of the medians generated using a permutation test under the null distribution. The observed statistic falls out of the 95% CI which means we can reject the H0 at the significance level of 0.05.", out.width='100%'}
